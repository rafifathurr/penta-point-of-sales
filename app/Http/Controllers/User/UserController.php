    ...
    public function dataTable()
    {
        ...
        $dataTable = DataTables::of($users)
            ...
            ->addColumn('action', function ($data) {
                ...
                $btn_action .= '<a href="' . route('user.edit', ['id' => $data->id]) . '" class="btn btn-warning d-flex align-items-center" title="Ubah"><i class="mdi mdi-lead-pencil mr-1"></i>Ubah</a>';
            ...
            ->only(['name', 'username', 'email', 'role', 'created_at', 'updated_at', 'action'])
            ->rawColumns(['action'])
            ->make(true);
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (is_null($username_check) && is_null($email_check)) {
                ...
                return redirect()
                    ->back()
                    ->with(['failed' => 'Email atau Username Telah Tersedia'])
                    ->withInput();
            }
        } catch (Exception $e) {
            return redirect()
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            if (is_null($username_check) && is_null($email_check)) {
                ...
                return redirect()
                    ->back()
                    ->with(['failed' => 'Email or Username Telah Tersedia'])
                    ->withInput();
            }
        } catch (Exception $e) {
            return redirect()
