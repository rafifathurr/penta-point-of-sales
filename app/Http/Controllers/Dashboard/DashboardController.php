...
class DashboardController extends Controller
{
    ...
    public function salesOrderDataTable(Request $request)
    {
        ...
        if (!is_null($request->year) && !is_null($request->month)) {
            ...
            $dataTable = DataTables::of($sales_order)
                ...
                ->only(['invoice_number', 'date', 'created_at', 'total_sell_price', 'total_capital_price', 'discount_price', 'grand_profit_price', 'grand_sell_price'])
    ...
    public function salesOrderExport(Request $request)
    {
        ...
        if (!is_null($request->year) && !is_null($request->month)) {
             ...
             * Get All Sales Order
             */
            $sales_order = SalesOrder::with(['paymentMethod', 'salesOrderItem.product'])
                ->whereNull('deleted_by')
                ->whereNull('deleted_at')
                ->whereMonth('created_at', $request->month)
                ->whereYear('created_at', $request->year)
            ...
            return Excel::download(new SalesOrderExport($data), 'Laporan_Penjualan_' . $request->year . '-' . $request->month . '_' . strtotime(date('Y-m-d H:i:s')) . '.xlsx');
        } else {
            return response()->json(['message' => 'Invalid Request'], 400);
        }
    }
