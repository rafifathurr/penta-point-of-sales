    ...
    public function dataTable()
    {
        ...
        $dataTable = DataTables::of($payment_method)
            ...
            ->addColumn('action', function ($data) {
                ...
                $btn_action .= '<a href="' . route('payment-method.edit', ['id' => $data->id]) . '" class="btn btn-warning d-flex align-items-center" title="Ubah"><i class="mdi mdi-lead-pencil mr-1"></i>Ubah</a>';
                $btn_action .= '<button class="btn btn-danger d-flex align-items-center ml-2" onclick="destroyRecord(' . $data->id . ')" title="Hapus"><i class="mdi mdi-delete mr-1"></i>Hapus</button>';
                $btn_action .= '</div>';
                return $btn_action;
            })
            ...
            if (is_null($slug_name_check)) {
                ...
                if ($payment_method) {
                    ...
                    return redirect()
                        ...
                        ->with(['success' => 'Berhasil Tambah Metode Pembayaran']);
                } else {
                    /**
                     * Failed Store Record
                     */
                    ...
                    return redirect()
                        ->back()
                        ->with(['failed' => 'Gagal Tambah Metode Pembayaran'])
                        ->withInput();
                }
            } else {
                return redirect()
            ...
            if (is_null($slug_name_check)) {
                ...
                if (!is_null($payment_method)) {
                    ...
                    if ($payment_method_update) {
                        ...
                        return redirect()
                            ->route('payment-method.index')
                            ->with(['success' => 'Berhasil Perbarui Metode Pembayaran']);
                    } else {
                        /**
                         * Failed Store Record
                         */
                        ...
                        return redirect()
                            ->back()
                            ->with(['failed' => 'Gagal Perbarui Metode Pembayaran'])
                            ->withInput();
                    }
                } else {
                    return redirect()
    ...
    public function destroy(string $id)
    {
        try {
            ...
            if ($payment_method_destroy) {
                DB::commit();
                session()->flash('success', 'Berhasil Hapus Metode Pembayaran');
            } else {
                /**
                 * Failed Store Record
                 */
                DB::rollBack();
                session()->flash('failed', 'Gagal Hapus Metode Pembayaran');
            }
        } catch (Exception $e) {
