<script type="text/javascript">
    ...
    $("#form_order").submit(function(e) {
        ...
        if ($("input[name='sales_order_item_check[]']").val() === undefined) {
            sweetAlertWarning('Harap Lengkapi Produk!');
        } else {
            Swal.fire({
    ...
    function addProduct(product) {
        ...
        if ($("#product_" + product).length == 0) {
            $.post("{{ route('product.getProduct') }}", {
                _token: token,
                product: product,
            }).done(function(data) {
                let sell_price_after_discount = 0;
                let discount = 0;

                sell_price_after_discount = data.sell_price;

                if (data.discount_price != null) {
                    discount = parseInt(data.sell_price) - parseInt(data.discount_price);
                    sell_price_after_discount = parseInt(data.discount_price);
                }

                let profit_price = parseInt(sell_price_after_discount) - parseInt(data.capital_price);
                let tr = $("<tr id='product_" + data.id + "'></tr>");
                let td_product = $("<td>" +
                    data.name +
                    "<input type='hidden' name = 'sales_order_item[" + data.id + "][id]'" +
                    "value = '" + data.id + "' > " +
                    "<input type='hidden' name = 'sales_order_item[" + data.id +
                    "][name]' value = '" + data.name + "' ></td>");
                let td_qty = $(
                    "<td>" +
                ...
                let td_price = $("<td align='right'>" +
                    ...
                    currencyFormat(sell_price_after_discount) +
                    "</span>" +
                    "<input type='hidden' id='total_sell_price_" + data.id +
                    "' name = 'sales_order_item[" +
                    data.id + "][total_sell_price]'" +
                    "value = '" + sell_price_after_discount + "' > " +
                ...
                // Append Tr Element
                (tr.append(td_product).append(td_qty).append(td_price).append(td_del));
                // Append To Table
    ...
    $("table#product").on("click", ".delete-row", function(event) {
    ...
    function validationQty(element, key) {
        ...
        let product_capital_price = $("#capital_price_" + key).val();
        let product_sell_price = $("#sell_price_" + key).val();
        let product_discount_price = $("#discount_" + key).val();

        let total_sell_price = 0;
        let total_discount_price = 0;

        if (parseInt(product_discount_price) > 0) {
            total_discount_price = parseInt(product_discount_price) * total_qty;
            total_sell_price = (parseInt(product_sell_price) * total_qty) - total_discount_price;
        } else {
            total_sell_price = parseInt(product_sell_price) * total_qty;
        }

        let total_profit_price = total_sell_price - (parseInt(product_capital_price) * total_qty);
